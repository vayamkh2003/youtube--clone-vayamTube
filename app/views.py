...
def home(request):
    ...
    videos_list = (
        Video.objects.select_related("channel", "user").all().order_by("-upload_time")
    )
...
def video(request, pk):
    try:
        video = get_object_or_404(
            Video.objects.select_related("channel", "user"), id=pk
        )
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            try:
                ...
                if not (channel and video_file and title and thumbnail):
                ...
                elif hasattr(video_file, "content_type") and not str(
                    ...
                    hasattr(thumbnail, "content_type")
                    and not str(thumbnail.content_type).startswith("image")
                ):
...
def searched(request):
    if request.method == "POST":
        ...
        if searched_value:
            videos_list = (
                Video.objects.filter(title__icontains=searched_value)
                .select_related("channel", "user")
                .order_by("-upload_time")
            )
            ...
            return render(
                request,
                "searched.html",
                {"videos": videos, "channels": channels, "search_term": searched_value},
            )
        else:
